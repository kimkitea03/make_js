...
function App() {
    ...
    const ChangeTest = (event) => {
        ...
        if (targetId === 'model') {
            ...
            //get(key) : key를 사용하여 해당 요소의 value를 반환합니다.
            const message = `${carMap.get(targetValue)}(${targetValue})`;


            setCar({ ...car, image: targetValue, model: targetValue, modleName: targetValue })

        } else if (targetId === 'color') {
            //전개연산자를 (...car) 사용하여 color 이외의 모든 데이터를 보존합니다.
            setCar({ ...car, color: targetValue });
        } else if (targetId === 'year') {
            setCar({ year: targetValue });
            let mycomment = '';
            if (targetValue === '2025') {
                mycomment = '신차입니다';
            ...
            setCar({ ...car, comment: targetValue });
    ...
    return (
        <div className="App">
            ...
            <p>
                ...
                색상의 {car.year}년산 {car.modleName}모델 <br />
                {car.comment}
            </p>
            <br /><br />
