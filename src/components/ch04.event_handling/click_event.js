function App() {
    ...
    const ClickEvent04 = (message, event) => {
        ...
        // 해당 이벤트가 보유하고 있는 속석(Property) 목록을 저장할 배열.
        const eventArray = [];
    ...
    return (
        <div className="App">
            ...
            {/* 매개 변수가 있는경우 화살표 함수로 둘러 싸야 합니다. */}
            <button onClick={() => ClickEvent02('김규식')}>매개 변수 1개</button><br />
            {/*  */}
            <button onClick={() => ClickEvent03(5, 7)}>매개 변수 2개</button><br />
            {/* 이벤트 객체를 사용하고자 하는 경우 */}
            ...
            <button id="cacel" onClick={(event) => ClickEvent05(event)}>취소</button>
            <br />
        </div>
    );
}
