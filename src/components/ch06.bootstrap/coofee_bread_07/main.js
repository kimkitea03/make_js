...
function App() {
    ...
    const [products, setProducts] = useState([
        ...
        { id: 6, name: "초콜릿 케이크", price: 6000, category: 'cake', stock: 677, image: 'chocolate_cake_01.png', description: "초콜릿으로 만든 케이크로, 풍부하고 진한 초콜릿 맛을 가지고 있습니다. 초콜릿으로 만든 케이크 스폰지와 초콜릿으로 만든 크림 또는 가나슈를 사용하여 제작됩니다." },
    ...
    const InsertData = (formData) => {
        // formData는 사용자가 입력한 폼 양식의 정보를 담고있는 객체입니다.
        console.log(`등록 버튼 누름(main)`);
        // console.log(`입력 값 : ${formData.xxx.value}`);

        //생성될 상품에 적용시킬 신규 아이디
        //기존에 있는것에 추가할떄는 ...은 필수인 듯 하다.
    ...
    */
    ...
    const Ordering = (orderInformation) => {
        ...
        // a와 b는 각각 상품 1개를 의미하는 객체 정보(java의 bean으로 봐도 무방함)
        products.sort((a, b) => {
            let aValue = a[column];
            let bValue = b[column];
            ...
            if (column === 'category') {
                const aCategory = categorys.find(c => c.english === a.category);
                const bCategory = categorys.find(c => c.english === b.category);

                aValue = aCategory ? aCategory.korean : a.category;
                bValue = bCategory ? bCategory.korean : b.category;
            }
            if (isCharacter) {//문자열 칼럼
    ...
    //사용자가 '정렬할 칼럼' 또는 '정렬 방식' 을 선택하면 여기 코드가 실행됩니다.
    ...
    // 사용자가 카테고리 콤보박스에서 다른 카테고리를 선택하였습니다.
    ...
    const isFilteringNeed = filterCategory && filterCategory !== 'all';

    //삼항 연산자를 사용하여 선택된 카테고리와 동일한 품목들만 필터링 합니다.
    const filteredProducts = isFilteringNeed
